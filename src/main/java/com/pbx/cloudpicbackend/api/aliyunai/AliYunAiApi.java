package com.pbx.cloudpicbackend.api.aliyunai;
...
public class AliYunAiApi {
     ...
     * @return
     */
    public CreateOutPaintingTaskResponse createOutPaintingTask(CreateOutPaintingTaskRequest createOutPaintingTaskRequest) throws MalformedURLException {
        if (createOutPaintingTaskRequest == null) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "扩图参数为空");
        }
        // 获取唯一键key
        String key = new URL(createOutPaintingTaskRequest.getInput().getImageUrl()).getPath();
        String url = cosManager.generateSimplePresignedDownloadUrl(key);
        createOutPaintingTaskRequest.setInput(new CreateOutPaintingTaskRequest.Input(url));
        // 发送请求
