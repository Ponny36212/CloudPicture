package com.pbx.cloudpicbackend.manager.websocket;
...
import com.pbx.cloudpicbackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.pbx.cloudpicbackend.manager.websocket.model.PictureEditActionEnum;
import com.pbx.cloudpicbackend.manager.websocket.model.PictureEditMessageTypeEnum;
import com.pbx.cloudpicbackend.manager.websocket.model.PictureEditRequestMessage;
import com.pbx.cloudpicbackend.manager.websocket.model.PictureEditResponseMessage;
...
public class PictureEditHandler extends TextWebSocketHandler {
     ...
     * @throws IOException
     */
    public void handleUserSaveMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        // 构造响应，发送用户保存的消息通知
        PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
        pictureEditResponseMessage.setType(PictureEditMessageTypeEnum.USER_SAVE.getValue());
        String message = String.format("用户 %s 保存了图片", user.getUserName());
        pictureEditResponseMessage.setMessage(message);
        pictureEditResponseMessage.setUser(userService.getUserVO(user));
        // 广播给所有用户
        broadcastToPicture(pictureId, pictureEditResponseMessage);
     ...
     * @throws IOException
     */
    public void handleQueryEditStatusMessage(PictureEditRequestMessage pictureEditRequestMessage,
                                             WebSocketSession session, User user, Long pictureId) throws IOException {
        // 构造响应
        PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
        pictureEditResponseMessage.setType(PictureEditMessageTypeEnum.EDIT_STATUS.getValue());
        ...
        if (editingUserId != null) {
            ...
            if (editingUser != null) {
                ...
                pictureEditResponseMessage.setMessage("当前有用户正在编辑");
            }
        } else {
            pictureEditResponseMessage.setMessage("当前没有用户正在编辑");
        }
